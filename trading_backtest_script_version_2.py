...
class BacktestEngine:
    """
    Core computation engine for the multi-asset trading backtest.
    Responsibilities:
        - Stores all configuration (assets, profiles, dates, capital settings).
        - Downloads OHLCV data from Yahoo Finance via yfinance.
        - Computes five technical indicators per asset (MA, RSI, BB, MACD, ATR).
        - Evaluates five BUY/SELL conditions and scores each bar.
        - Simulates bar-by-bar trade execution with commission, slippage,
          and dynamic ATR-based stop-loss.
        - Returns a standardised ``results`` list consumed by all other classes.
    Parameters:
        assets : dict[str, str]
            Display name → Yahoo Finance ticker mapping.
        profiles : dict[str, dict]
            Parameter sets keyed by profile name (COMMODITY, TECH, …).
        asset_profiles : dict[str, str]
            Maps each asset name to its profile name.
        start_date : str
            Backtest start date in ``"YYYY-MM-DD"`` format.
        end_date : str, optional
            Backtest end date; defaults to today.
        self.initial_cap : float
            Starting capital in USD per asset (default 10 000).
        commission : float
            Round-trip commission rate per trade (default 0.001 = 0.1 %).
        slippage : float
            Slippage rate applied on entry and exit (default 0.0005 = 0.05 %).
    """
    def __init__(self, assets: dict, profiles: dict, asset_profiles: dict, start_date: str = "2021-01-01", end_date: str = None, initial_cap: float = 10_000, commission: float = 0.001, slippage: float = 0.0005,):
        self.assets = assets
    ...
    def compute_indicators(self, df: pd.DataFrame, p: dict) -> pd.DataFrame:
        """
    ...
    def generate_signals(self, df: pd.DataFrame, p: dict) -> pd.DataFrame:
        """
            Score each bar with five binary BUY conditions and their SELL inverses.
            BUY conditions (signal = 1 when score >= 3)
            1. EMA_short > EMA_long   (trend direction)
            2. RSI < RSI_mid          (room to grow)
            3. BB_pct < 0.4           (near lower band)
            4. MACD > MACD_signal     (momentum positive)
            5. Close > SMA_short      (price above short average)
            Columns added: ``buy_score``, ``sell_score``, ``signal`` (1 / -1 / 0).
        """
        c = df["Close"].astype(float)
        ...
        df["signal"] = 0
        df.loc[df["buy_score"] >= 3, "signal"] = 1; df.loc[df["sell_score"] >= 3, "signal"] = -1
        return df

    # BACKTEST ENGINE (final value, profit, Buy&Hold, Alpha, Win rate, Sharpe, Max Drawdown, Profit Factor, etc.)
    def run_backtest(self, df: pd.DataFrame, asset_name: str, p: dict) -> dict:
        """
        ...
        if position > 0:
            ...
            proceeds = position * last_px * (1 - self.commission)
            pnl = proceeds - position * entry_px
            capital += proceeds
        ...
        # Statistics
        final_val = capital
        total_return = (final_val - self.initial_cap) / self.initial_cap * 100
        # Buy & Hold benchmark
    ...
    def compute_results_from_assets(self) -> list:
        """
        Main method to compute backtest results for all assets.
        For each asset:
            1. Download data from Yahoo Finance.
            2. Compute indicators.
            3. Generate signals.
            4. Run backtest simulation.
        Returns a list of result dicts for each asset.
        """
        print("  MULTI-ASSET TRADING ALGORITHM BACKTEST")
        print(f"  Period: {self.start_date}  →  {self.end_date}")
        print(f"  Initial capital: ${self.initial_cap:,.0f} / asset")
        ...
        for name, ticker in self.assets.items():
            ...
            try:
                raw = yf.download(ticker, start=self.start_date, end=self.end_date, progress=False, auto_adjust=True)
                if raw.empty:
                    print(f"⚠  No data for {name}, skipping.")
                    continue
                raw = self._flatten_columns(raw)
                profile_name = self.asset_profiles.get(name, "TECH"); p = self.profiles[profile_name]
                if len(raw) < p["MA_LONG"] + 10:
                    print(f"⚠  Insufficient data for {name}, skipping.")
                    continue
                df = self.compute_indicators(raw.copy(), p)
                df = self.generate_signals(df, p)
                res = self.run_backtest(df, name, p)
    ...
    def analyze_asset(self, name: str, interval: str = "4h"):
        """
        Quick analysis of one asset – download current data
        and display status of all indicators + recommendation to terminal.
        Running:
        ...
        try:
            if interval == "4h":
                ...
                raw = self._flatten_columns(raw)
            if raw.empty:
                print(f"x  Failed to download data for {name}.")
                return
            min_bars = max(p["MA_LONG"] + 5, 50)
            if len(raw) < min_bars:
                print(f"x  Insufficient data ({len(raw)} candles, need {min_bars}).")
                print(f"x  Try longer period or different interval.")
                return
        except Exception as e:
            print(f"x  Download error: {e}")
            return
        df = self.compute_indicators(raw.copy(), p); last = df.iloc[-1]; c = df["Close"].astype(float)
        ...
        tp2 = bb_upper
        risk_usd = risk_per * (self.initial_cap * 0.95 / buy_limit) if buy_limit > 0 else 0
        ...
        print(f"Risk/trade:  ${risk_usd:>9,.0f}  (at ${self.initial_cap:,} capital)")
        # Context – where are we in BB band
        bb_mid = float(df["BB_mid"].iloc[-1]) if not pd.isna(df["BB_mid"].iloc[-1]) else price
        print(f"\nCONTEXT:")
        ...
        print(f"MACD:      Histogram {hist_trend} – trend {'strengthens, hold position' if macd_hist > 0 else 'weakens, be cautious'}")
        print()

class MonteCarloForecastSimulator:
    """
    Monte Carlo simulator for forecasting future price paths based on historical data and profile-specific models.
    Each profile has a different underlying model to capture typical behavior:
        - DEFENSIVE: simple random walk (base case).
        - TECH: random walk with random earnings jumps.
        - COMMODITY: Geometric Brownian Motion with mean reversion.
        - CRYPTO: GARCH(1,1) volatility model.
        - FOREX_IDX: Ornstein-Uhlenbeck mean reversion.
    Parameters:
        mc_days        : Forecast horizon in trading days (default 30).
        mc_simulations : Number of simulation paths (default 1 000).
    """
    MC_PROFILE_META = {
    ...
    def __init__(self, mc_days: int = 30, mc_simulations: int = 1000):
        self.mc_days        = mc_days
        self.mc_simulations = mc_simulations

    # PROFILE-SPECIFIC SIMULATION MODELS

    def _mc_random_walk(self, returns, last_price, n_sim, n_days, rng):
    ...
    def _mc_random_walk_earnings(self, returns, last_price, n_sim, n_days, rng):
    ...
    def _mc_gbm_mean_reversion(self, close, last_price, n_sim, n_days, rng, lookback=252):
    ...
    def _mc_garch(self, returns, last_price, n_sim, n_days, rng):
    ...
    def _mc_ornstein_uhlenbeck(self, close, last_price, n_sim, n_days, rng, lookback=252):
        """FOREX_IDX – Ornstein-Uhlenbeck (mean reversion).
    ...
    # public methods
    def forecast(self, close: pd.Series, profile: str = "TECH", lookback: int = 90) -> dict:
        """
        Run Monte Carlo simulation for *close* using the model for *profile*.
        Parameters:
            close:      Price series (Close column from a price DataFrame).
            profile:    Asset profile name; selects the simulation model.
            lookback:   Number of historical bars used to calibrate the model.
        Returns:
            dict with keys: dates, p10, p25, p50, p75, p90, last, profile.
        """
        n_sim = self.mc_simulations; n_days = self.mc_days
        returns = close.iloc[-lookback:].pct_change().dropna(); last_price = float(close.iloc[-1])
        rng = np.random.default_rng(42)
        if profile == "DEFENSIVE":
            paths = self._mc_random_walk(returns, last_price, n_sim, n_days, rng)
        elif profile == "TECH":
            paths = self._mc_random_walk_earnings(returns, last_price, n_sim, n_days, rng)
        elif profile == "COMMODITY":
            paths = self._mc_gbm_mean_reversion(close.iloc[-lookback:], last_price, n_sim, n_days, rng)
        elif profile == "CRYPTO":
            paths = self._mc_garch(returns, last_price, n_sim, n_days, rng)
        elif profile == "FOREX_IDX":
            paths = self._mc_ornstein_uhlenbeck(close.iloc[-lookback:], last_price, n_sim, n_days, rng)
        else:
            paths = self.mc_random_walk(returns, last_price, n_sim, n_days, rng)
    ...
    def draw_chart(self, ax, close: pd.Series, profile: str = "TECH"):
        """
        ...
        """
        mc = self.forecast(close, profile=profile); d = mc["dates"] 
        meta = self.MC_PROFILE_META.get(profile, self.MC_PROFILE_META["TECH"]); color = meta["color"]; short = meta["short"]
        ...
        ax.plot(d, mc["p50"], color=color, lw=1.8, ls="--", label=f"MC median ({short}, {self.mc_days}d)", zorder=4)
...
class GraphPlotter:
    """
        Visualisation engine – produces and saves all PNG chart files.
        Parameters
        mc_engine : MonteCarloEngine
            Injected MC engine used to draw forecast fan charts inside price panels.
            Injecting it (rather than instantiating internally) makes it easy to
            substitute a differently configured engine or a mock in tests.
        self.initial_cap : float
            Baseline capital line drawn on equity curves (default 10 000).
    """  
    def __init__(self, mc_engine: MonteCarloForecastSimulator, initial_cap: float = 10_000):
        self.mc = mc_engine
        self.initial_cap = initial_cap

    def _draw_price_panel(self, ax, df, close, trades, p, zoom: bool =False):
    ...
    def plot_asset(self, result: dict, save_path: str = None):
        """
        ...
        Parameters:
            result    : Result dict from BacktestEngine.run_backtest.
            save_path : Output file path; auto-generated with timestamp if None.
        """
        df = result["price_df"]; eq = result["equity_df"]; trades = result["trades_df"]
        close  = df["Close"].astype(float)
        name = result["asset"]; p = result["p"]; profile = result.get("profile", "TECH")
        
        zoom_start = df.index[-1] - pd.DateOffset(months=6)
        df_z = df[df.index >= zoom_start]; close_z = close[close.index >= zoom_start]; eq_z = eq[eq.index >= zoom_start]
        
        ts = datetime.now().strftime("%d.%m.%Y  %H:%M:%S")
        mc_meta = self.mc.MC_PROFILE_META.get(profile, self.mc.MC_PROFILE_META["TECH"])
        ...
        self._draw_price_panel(ax1, df, close, trades, p, zoom=False)
        self.mc.draw_chart(ax1, close, profile=profile)  
        ax1.set_title(f"Price + MA + Bollinger Bands + Trades  (full period) | MC {self.mc.mc_simulations}x / {self.mc.mc_days}d  [{mc_meta['label']}]", fontsize=10)
        ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m"))
        # Price zoom     
        ax1z = fig.add_subplot(gs[0, 1])
        self._draw_price_panel(ax1z, df_z, close_z, trades, p, zoom=True)
        self.mc.draw_chart(ax1z, close_z, profile=profile)
        ax1z.xaxis.set_major_formatter(mdates.DateFormatter("%m/%y"))
        ax1z.set_title(f"Zoom 6M  +  MC forecast 30d  [{mc_meta['short']}]", fontsize=10, color="#1565c0")
        ...
        ax2.axhline(self.initial_cap, color="gray", ls="--", lw=0.8, label="Initial capital")
        ax2.fill_between(eq.index, self.initial_cap, eq["equity"], where=eq["equity"] >= self.initial_cap, alpha=0.2, color="#00c853")
        ax2.fill_between(eq.index, self.initial_cap, eq["equity"], where=eq["equity"] <  self.initial_cap, alpha=0.2, color="#d50000")
        ...
        ax2z.axhline(self.initial_cap, color="gray", ls="--", lw=0.8)
        ax2z.fill_between(eq_z.index, self.initial_cap, eq_z["equity"], where=eq_z["equity"] >= self.initial_cap, alpha=0.2, color="#00c853")
        ax2z.fill_between(eq_z.index, self.initial_cap, eq_z["equity"], where=eq_z["equity"] <  self.initial_cap, alpha=0.2, color="#d50000")
    ...
    def plot_summary(self, results: list):
        ...
        ax.set_xticks(range(len(names))); ax.set_xticklabels(names, rotation=45, ha="right", fontsize=8)
        ax.set_ylabel("Max Drawdown (%)"); ax.set_title("Maximum capital decline")
...
class ExportGenerator:
    """
    Export engine – produces PNG summary tables and prints terminal reports.
    Receives ``results`` in the constructor (output of BacktestEngine.run_all).
    Also handles the ``--signals-hourly`` mode independently of a full backtest
    (pass ``results=[]`` in that case).
    Parameters:
        results       : list[dict] from BacktestEngine.run_all (may be empty).
        assets        : Same asset dict forwarded from BacktestEngine.
        asset_profiles: Same profile-mapping dict forwarded from BacktestEngine.
        profiles      : Same profiles dict forwarded from BacktestEngine.
        initial_cap   : Capital baseline used for risk-per-trade calculation.
    """
    def __init__(self, results: list, assets: dict, asset_profiles: dict, profiles: dict, initial_cap: float = 10_000,):
    ...
    def _safe(val, default=0.0):
        """Return float(val) or default if val is NaN/None."""
        return float(val) if not pd.isna(val) else default

    def _compute_indicators_and_signals(self, df: pd.DataFrame , p: dict):
        """
        Standalone indicator + signal computation used by intraday methods.
        Does not depend on a BacktestEngine instance so ExportManager can
        work with any downloaded DataFrame at any interval.
        Returns:
            df       : DataFrame enriched with all indicator columns.
            conds_buy: Dict of five bool conditions evaluated on the last bar.
        """
        # Reuse BacktestEngine static logic inline to stay self-contained
        c = df["Close"].astype(float)
        df["SMA_short"] = c.rolling(p["MA_SHORT"]).mean(); df["SMA_long"] = c.rolling(p["MA_LONG"]).mean()
        df["EMA_short"] = c.ewm(span=p["MA_SHORT"], adjust=False).mean(); df["EMA_long"] = c.ewm(span=p["MA_LONG"], adjust=False).mean()
        ...
        rsi_mid = (p["RSI_OB"] + p["RSI_OS"]) / 2
        conds_buy = {
            "MA":   self._safe(last["EMA_short"], price) > self._safe(last["EMA_long"], price),
            "RSI":  self._safe(last["RSI"], 50)  < rsi_mid,
            "BB":   self._safe(last["BB_pct"], 0.5) < 0.4,
            "MACD": self._safe(last["MACD"], 0) > self._safe(last["MACD_sig"], 0),
            "ATR":  price > self._safe(last["SMA_short"], price),
        }
        return df, conds_buy
    
    def _signal_label(self, buy_score: int, sell_score: int) -> str:
        if buy_score >= 3:
            return "BUY"
        if sell_score >= 3:
            return "SELL"
    ...
    def _order_levels(self, price, bb_lower, bb_upper, atr, p):
        """Compute Buy Limit, Stop-Loss, TP1, TP2 and risk USD."""
        buf = 0.005
        buy_limit = bb_lower * (1 + buf)
        stop_loss = buy_limit - p["ATR_SL_MULT"] * atr
        risk_per = buy_limit - stop_loss
        tp1 = buy_limit + risk_per
        qty_est   = (self.initial_cap * 0.95) / buy_limit if buy_limit > 0 else 0
        risk_usd = risk_per * qty_est
        return buy_limit, stop_loss, tp1, bb_upper, risk_usd

    def export_table_png(self, table: list, headers: list):
        """Export summary results table to PNG file."""
        ts_label = datetime.now().strftime("%d.%m.%Y  %H:%M:%S")
        fname = f"summary_table.png"
        n_rows, n_cols = len(table), len(headers)
        # Cell colors
        ...
        fig, ax = plt.subplots(figsize=(max(18, n_cols * 1.5), 1.2 + n_rows * 0.42))
        ax.axis("off")
        ...
        for j in range(n_cols):
            cell = tbl[0, j]; cell.set_text_props(color="white", fontweight="bold"); cell.set_facecolor("#1E50A0")
    ...
    def _render_table_png(self, rows: list, headers: list, fname: str, title: str, signal_col: int = 3, custom_colors: dict = None, scale_y: float = 1.8):
        """
        Generic matplotlib table renderer used by all export methods.
        Parameters:
            rows          : List of row lists (strings).
            headers       : Column header list.
            fname         : Output file path.
            title         : Figure suptitle text.
            signal_col    : Column index whose value drives BUY/SELL/NEU row colour.
            custom_colors : Optional dict {col_index: hex_color} for column overrides.
            scale_y       : Table row height scale factor.
        """
        ...
        for i, row in enumerate(rows):
            ...
            for j in range(n_cols):
                ...
                elif j in custom_colors:
                    row_c.append(custom_colors[j])
                else:
                    row_c.append(bg)
            cell_colors.append(row_c)
        ...
        tbl.auto_set_font_size(False); tbl.set_fontsize(8.5); tbl.scale(1, scale_y)
        for j in range(n_cols):
            tbl[0, j].set_text_props(color="white", fontweight="bold")
        for i in range(1, n_rows + 1):
            tbl[i, signal_col].set_text_props(fontweight="bold")
        fig.suptitle(title, fontsize=10, fontweight="bold", y=0.99, color="#1E50A0")
        plt.tight_layout()
        plt.savefig(fname, dpi=150, bbox_inches="tight")
        plt.close()

    def compute_yearly_breakdown(self) -> dict:
        """
        Compute per-year performance metrics for every asset in ``self.results``.
        Returns:
        ...
        for r in self.results:
            ...
            for yr in sorted(eq.index.year.unique()):
                ...
                sharpe_yr = (daily_ret.mean() / daily_ret.std() * np.sqrt(252) if daily_ret.std() > 0 else 0)
                # Yearly win rate
    ...
    def export_signals_png(self):
        """Export current signals and price levels to PNG table."""
        ts_label = datetime.now().strftime("%d.%m.%Y  %H:%M:%S")
        fname  = f"signals.png"
        rows = []
        for r in self.results:
            df, conds_buy = self._compute_indicators_and_signals(r["price_df"].copy(), r["p"])
            last = df.iloc[-1]
            price = float(df["Close"].astype(float).iloc[-1])
            atr = self._safe(last["ATR"])
            bb_lower = self._safe(last["BB_lower"], price)
            bb_upper = self._safe(last["BB_upper"], price)
            buy_score = sum(conds_buy.values())
            sell_score = sum(not v for v in conds_buy.values())
            signal = self._signal_label(buy_score, sell_score)
            stop_loss = price - r["p"]["ATR_SL_MULT"] * atr
            
            def _ic(v): return "✔" if v else "×"
            icons = f'{_ic(conds_buy["MA"])}    {_ic(conds_buy["RSI"])}    {_ic(conds_buy["BB"])}    {_ic(conds_buy["MACD"])}    {_ic(conds_buy["ATR"])}'

            sl_pct = (price - stop_loss) / price * 100
            tp     = price + 2 * r["p"]["ATR_SL_MULT"] * atr
            tp_pct = (tp - price) / price * 100
            st_pct = (bb_upper - price) / price * 100
        ...
        self._render_table_png(
            ...
            fname=fname,
            title = f"CURRENT SIGNALS AND PRICE LEVELS | Generated: {ts_label}",
            signal_col=3,
        )
        print(f"→ Signal table exported: {fname}")
        
    def export_order_levels_png(self):
        """
        ...
        for r in self.results:
            ...
            price = float(df["Close"].astype(float).iloc[-1])
            atr = self._safe(last["ATR"])
            bb_lower = self._safe(last["BB_lower"], price * 0.95)
            bb_upper = self._safe(last["BB_upper"], price * 1.05)
            buy_score = sum(conds_buy.values())
            sell_score = sum(not v for v in conds_buy.values())
            signal = self._signal_label(buy_score, sell_score)
            bl, sl, tp1, tp2, risk_usd = self._order_levels(price, bb_lower, bb_upper, atr, r["p"])
            risk_per = bl - sl

            def pct(target): return (target - price) / price * 100
            def rr(target): return abs((target - bl) / risk_per) if risk_per > 0 else 0

            rows.append([
                r["asset"], r.get("profile", "-"), f"${price:,.2f}", signal, 
                f"${bl:,.2f} ({pct(bl):+.1f}%)",
                f"${sl:,.2f} ({pct(sl):+.1f}%)",
                f"${tp1:,.2f} ({pct(tp1):+.1f}%) 1:{rr(tp1):.1f}",
                f"${tp2:,.2f} ({pct(tp2):+.1f}%) 1:{rr(tp2):.1f}",
                f"${risk_usd:,.0f}",
            ])
        n_cols = 9
        custom_colors = {4: "#e8f5e9", 5: "#fdecea", 6: "#e3f2fd", 7: "#e3f2fd", 8: "#fffde7"}
        self._render_table_png(
            rows,
            headers = [
                "Asset", "Profile", "Price", "Signal",
                "Buy Limit (BB low+0.5%)", "Stop-Loss (ATR x mult.)",
                "Take Profit 1 (R:R 1:1)", "Take Profit 2 (BB upper)",
                "Risk/trade USD"
            ],
            fname=fname,
            title=(f"RECOMMENDED PRICE ORDERS  |  Generated: {ts_label}  | Buy Limit = BB low+0.5% | SL = BuyLim - ATR x mult. | TP1 = R:R 1:1 | TP2 = BB upper"),
            signal_col=3,
    ...
    def print_current_signals(self):
        ...
        for r in self.results:
            ...
            sell_score = sum(not v for v in conds_buy.values())
            signal     = self._signal_label(buy_score, sell_score)
            signal_str = {"BUY": "✔  ACTIVE BUY", "SELL": "x  ACTIVE SELL", "NEU": "o  NEUTRAL"}.get(signal, "o  NEUTRAL")
            # Price levels
            ...
            print(f"  {'─'*68}")
            print(f"  {'Indicator':<16} {'Value':>12}   {'BUY?':^5}   {'Detail'}")
            print(f"  {'':-<16} {'':-<12}   {'':-<5}   {'':-<30}")
            ema_s = self._safe(last["EMA_short"], price)
            ema_l = self._safe(last["EMA_long"],  price)
            sma_s = self._safe(last["SMA_short"], price)
            rsi = self._safe(last["RSI"], 50)
            bb_p = self._safe(last["BB_pct"], 0.5)
            macd = self._safe(last["MACD"])
            msig = self._safe(last["MACD_sig"])
            rsi_mid = (p["RSI_OB"] + p["RSI_OS"]) / 2
            rows_ind = [
                ("MA Crossover", f"EMA{p['MA_SHORT']}={'>' if ema_s>ema_l else '<'}EMA{p['MA_LONG']}", conds_buy["MA"], f"EMA{p['MA_SHORT']}={ema_s:.2f}  EMA{p['MA_LONG']}={ema_l:.2f}"),
                ("RSI",          f"{rsi:.1f}", conds_buy["RSI"], f"< {rsi_mid:.0f} for BUY"),
                ("Bollinger",    f"BB%={bb_p:.2f}", conds_buy["BB"], f"low={bb_lower:.2f}  up={bb_upper:.2f}"),
                ("MACD",         f"{'MACD>Sig' if macd>msig else 'MACD<Sig'}", conds_buy["MACD"], f"MACD={macd:.3f}  Sig={msig:.3f}"),
                ("ATR trend",    f"{'price>SMA' if price>sma_s else 'price<SMA'}", conds_buy["ATR"], f"Price={price:.2f}  SMA={sma_s:.2f}"),
            ]
            for ind, val, is_buy, det in rows_ind:
                icon = "✔" if is_buy else "x"
                print(f"  {ind:<16} {val:>12}   {icon}     {det}")
            print(f"{'─'*68}")
            print(f"BUY score: {buy_score}/5   SELL score: {sell_score}/5")
            print(f"BUY zone:      ${bb_lower:>10.2f}  –  ${price:.2f}  (BB lower – current price)")
            print(f"Stop-Loss:     ${stop_loss:>10.2f}   ({sl_pct:.1f} below price)")
            print(f"Take Profit:   ${take_profit:>10.2f}   (+{tp_pct:.1f}%, R:R 1:2)")
            print(f"SELL target:   ${bb_upper:>10.2f}   (+{st_pct:.1f}%, BB upper)")
        print()

    def run_hourly_signals(self, interval: str = "1h"):
        """
        Download fresh intraday data for all assets, compute indicators, and
        export a PNG signal table without running a full backtest.
        Falls back gracefully to an N/A row if Yahoo Finance returns
        insufficient data for an asset (e.g. illiquid Czech stocks).
        Parameters:
            interval: One of the INTERVAL_SETTINGS keys (default "1h"). "4h" is resampled from hourly data.
            Output file: ``signals_<interval>.png``
        """
        if interval not in INTERVAL_SETTINGS:
            print(f"\nx  Unknown interval '{interval}'.")
            print(f"   Available: {', '.join(INTERVAL_SETTINGS.keys())}")
            return
        ...
        for name, ticker in ASSETS.items():
            ...
            try:
                ...
                if raw.empty or len(raw) < max(p["MA_LONG"] + 5, 50):
                    ...
                    continue
                df, conds_buy = self._compute_indicators_and_signals(raw.copy(), p)
                last     = df.iloc[-1]
                c        = df["Close"].astype(float)
                price    = float(c.iloc[-1])
                prev     = float(c.iloc[-2])
                ...
                bb_upper = self._safe(last["BB_upper"], price)
                rsi      = self._safe(last["RSI"], 50)
                macd_h   = self._safe(last["MACD_hist"])
                buy_score  = sum(conds_buy.values())
                sell_score = sum(not v for v in conds_buy.values())
                signal     = self._signal_label(buy_score, sell_score)
                bl, sl, tp1, tp2, risk_usd = self._order_levels(price, bb_lower, bb_upper, atr, p)
                arrow = "▲" if change >= 0 else "▼"
                ...
                rows.append([
                    name, profile_name, f"${price:,.2f} {arrow}{change:+.2f}%", 
                    signal, f"{buy_score}/5", f"{sell_score}/5", ind_icons,
                    f"${bl:,.2f}",
                    f"${sl:,.2f} ({pct(sl):.1f}%)",
                    f"${tp1:,.2f} ({pct(tp1):+.1f}%)",
                    f"${bb_upper:,.2f} ({pct(bb_upper):+.1f}%)",
                    f"RSI:{rsi:.0f}  MACD:{'▲' if macd_h>=0 else '▼'}",
                ])
            except Exception as e:
                print(f"x {e}")
        ...
        self._render_table_png(
            rows,
            headers = [
                "Asset", "Profile", f"Price ({interval})", "Signal",
                "BUY sc.", "SELL sc.", "MA / RSI / BB / MACD / ATR",
                "Buy Limit", "Stop-Loss", "Take Profit 1", 
                "SELL target (BB upper)","RSI / MACD trend",
            ],
            fname=fname,
            title=(f"HOURLY SIGNAL ANALYSIS  |  Interval: {interval} ({iv['label']})"
                   f"  |  Generated: {ts_label}"),
            signal_col=3,
            scale_y=2.0,
        
        )
        print(f"\n  -> PNG table saved: {fname}")


# MAIN PROGRAM
def main():
    engine = BacktestEngine(
        assets=ASSETS, profiles=PROFILES, asset_profiles=ASSET_PROFILES,
        start_date="2021-01-01", 
        initial_cap=10_000, # USD per asset
        commission=0.001, # 0.1% per trade
        slippage=0.0005, # 0.05%
    )
    mc = MonteCarloForecastSimulator(mc_days=30, mc_simulations=1000)
    plotter = GraphPlotter(mc_engine=mc, initial_cap=engine.initial_cap)
    results = engine.compute_results_from_assets()
    if not results:
        print("\nx  No data downloaded.")
        return
    export = ExportGenerator(
        results=results, assets=ASSETS,
        asset_profiles=ASSET_PROFILES, profiles=PROFILES,
        initial_cap=engine.initial_cap,
    )
    table = []
    for r in results:
        table.append([
    ...
    #export.print_yearly_breakdown()
    #export.print_current_signals()
    export.export_signals_png()
    export.export_order_levels_png()
    export.export_table_png(table, headers)
    # Summary comparison chart
    for res in results:
        plotter.plot_asset(res, save_path=f"chart_{res['asset'].lower()}.png")
...
if __name__ == "__main__":
    ...
    engine = BacktestEngine(
        assets=ASSETS, profiles=PROFILES, asset_profiles=ASSET_PROFILES,
        start_date="2021-01-01", initial_cap=10_000,
    )
    if args and args[0] == "--analyze":
        if len(args) < 2:
            ...
            engine.analyze_asset(args[1], interval=_get_interval(args, default="1d"))
    elif args and args[0] == "--signals-hourly":
        interval = _get_interval(args, "1h")
        export   = ExportGenerator(results=[], assets=ASSETS, asset_profiles=ASSET_PROFILES, profiles=PROFILES)
        export.run_hourly_signals(interval=interval)
    else:
        main()
